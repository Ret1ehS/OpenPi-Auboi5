...
class TUIConfig:
    ...
    trt_denoise: bool = False  # TUI-level TRT switch; enables denoise and vision fastpaths together.
...
class CollectTUIConfig:
    ...
    task: str              # "pick_and_place" | "open_and_close" | "keyboard_teleop" | "storage" | "teach_pendant"
    save_fps: int          # 30 | 50
    state_mode: str        # "yaw"
    quit: bool = False
    ...
    *,
    ...
    rows: list[MenuRow] = [
        ...
        ToggleItem(
            ...
            selected=1
            if (
                _get_env_bool("OPENPI_PYTORCH_TRT_DENOISE", False)
                or _get_env_bool("OPENPI_PYTORCH_TRT_VISION", False)
            )
            else 0,
        ),
...
def _snapshot_config(rows: list[MenuRow]) -> TUIConfig:
    ...
    return TUIConfig(
        ...
        trt_denoise=_get_toggle(rows, "TRT") == "true",
...
def _collect_disabled_labels(rows: list[MenuRow]) -> set[str]:
    ...
    task = _get_toggle(rows, "Task")
    if _get_toggle(rows, "Mode") != "auto" or task in {"open_and_close", "keyboard_teleop", "storage", "teach_pendant"}:
        disabled.add("Auto Episodes")
    return disabled


def _is_manual_locked_task(rows: list[MenuRow]) -> bool:
    return _get_toggle(rows, "Task") in {"open_and_close", "keyboard_teleop", "storage", "teach_pendant"}
...
def run_collect_tui_config(
    *,
    ...
    rows: list[MenuRow] = [
        ...
        ToggleItem(
            "Task",
            ["pick_and_place", "open_and_close", "keyboard_teleop", "storage", "teach_pendant"],
